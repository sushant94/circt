...
private:
  ...
  // attach the result to, for example when resetting a register driven by a
  // block argument.
  size_t genIte(size_t condLID, size_t tLID, size_t fLID, int64_t width) {
    size_t opLID = lid++;
    size_t sid = sortToLIDMap.at(width);
    os << opLID << " "
       << "ite"
  ...
  void genNext(Value next, Operation *reg, int64_t width) {
    genNext(getOpLID(next), reg, width);
  }
  ...
  void finalizeRegVisit(Operation *op) {
    ...
    if (reset) {
      ...
      if (auto *nextOp = next.getDefiningOp()) {
        setOpLID(nextOp);
        genIte(nextOp, resetLID, resetValLID, nextLID, width);
        finalNextLID = getOpLID(nextOp);
      } else {
        finalNextLID = genIte(resetLID, resetValLID, nextLID, width);
      }
    } else {
...
public:
  ...
  void visitHWOutput(Operation* op) {
    ...
    for (Value operand : op->getOperands()) {
      if (isa<seq::ClockType, seq::ImmutableType>(operand.getType()))
        continue;

      if (outputIdx >= outputs.size()) {
        op->emitError("output port metadata did not match hw.output operands");
      ...
      size_t new_lid = lid++;
      StringRef name = outputs[outputIdx++];
