...
public:
  ...
  ConvertHWToBTOR2PPPass(raw_ostream &os, bool formalNormalizeRegs = false)
      : os(os) {
    this->formalNormalizeRegs = formalNormalizeRegs;
  }
  ConvertHWToBTOR2PPPass(const ConvertHWToBTOR2PPPass &other)
      : ConvertHWToBTOR2PPPass(other.os, other.formalNormalizeRegs) {}
...
private:
  ...
  void genNext(Value next, Operation *reg, int64_t width) {
    genNext(getOpLID(next), getOpLID(reg), width);
  }
  ...
  size_t getValueLID(Value value) { return getOpLID(value); }
...
circt::createConvertHWToBTOR2PPPass(llvm::raw_ostream &os,
  ...
  return std::make_unique<ConvertHWToBTOR2PPPass>(os, formalNormalizeRegs);
}
